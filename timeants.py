...
class antsystem:
	...
	def itertwoexchange(self, n, choices):
		"""
		Written by Daniel Webber and Will Longabaugh
		"""
		lastchoices = []
		for i in range(n):
			lastchoices = choices
	...
	def genants(self):
		...
		for x in range(self.antnum):
			ants.append(ant.Ant(self.alpha, self.distances, self.flows, self.n))
		return ants
...
doit = antsystem(x, %i, %i, %f, %f, bestscore)
